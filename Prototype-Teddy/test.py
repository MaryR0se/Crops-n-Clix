...
'''''''''
    ...
    def unlock(self):
        if self.current_animal.num_animals == 1:
            if (len(self.all_animals) +1 ) < len(self.all_animals_count):
                add = self.all_animals_count[self.update_count]
                self.all_animals.append(add)
                print("updated", add)
