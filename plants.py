...
class Game:
    def __init__(self):
        ...
        #Plant Images
        self.green_young_img = pygame.image.load(os.path.join("Assets", "FernYoung.png"))
        self.green_mature_img = pygame.image.load(os.path.join("Assets", "FernMature.png"))
        self.green_young = pygame.transform.scale(self.green_young_img, (80, 80))
        self.green_mature = pygame.transform.scale(self.green_mature_img, (80, 80))

        self.blue_young_img = pygame.image.load(os.path.join("Assets", "BlueYoung.png"))
        self.blue_mature_img = pygame.image.load(os.path.join("Assets", "BlueMature.png"))
        self.blue_young = pygame.transform.scale(self.blue_young_img, (80, 80))
        self.blue_mature = pygame.transform.scale(self.blue_mature_img, (80, 80))

        self.pink_young_img = pygame.image.load(os.path.join("Assets", "PinkYoung.png"))
        self.pink_mature_img = pygame.image.load(os.path.join("Assets", "PinkMature.png"))
        self.pink_young = pygame.transform.scale(self.pink_young_img, (80, 80))
        self.pink_mature = pygame.transform.scale(self.pink_mature_img, (80, 80))

        self.weed_young_img = pygame.image.load(os.path.join("Assets", "WeedYoung.png"))
        self.weed_mature_img = pygame.image.load(os.path.join("Assets", "WeedMature.png"))
        self.weed_young = pygame.transform.scale(self.weed_young_img, (80, 80))
        self.weed_mature = pygame.transform.scale(self.weed_mature_img, (80, 80))
        ...
        #list of plants: starting color, then mature color, then low time to mature, then high time, then counter, then plant name, then plant young, then plant mature
        self.plants = [[LIGHT_GREEN, DARK_GREEN, 1200, 1500, 0, 'Green Plants', self.green_young, self.green_mature], 
        [LIGHT_BLUE, DARK_BLUE, 3000, 4000, 0, 'Blue Plants', self.blue_young, self.blue_mature], 
        [LIGHT_PURPLE, DARK_PURPLE, 300, 600, 0, 'Pink Plants', self.pink_young, self.pink_mature], 
        [BLACK, WHITE, 700, 800, 0, 'Weeds', self.weed_young, self.weed_mature]] 
    ...
    def current_plant(self, i, j):
        for a in self.plants:
            if a[0] == self.garden_colors[i][j]:
                return a[6]
            elif a[1] == self.garden_colors[i][j]:
                return a[7]
    ...
    def draw_screen(self):
        ...
        for i in range(7):
            for j in range(4):
                pygame.draw.rect(screen, DARK_BROWN, self.garden_tiles[i][j])
                if self.garden_colors[i][j] != DARK_BROWN:
                    plant_rect = pygame.Rect(60+i*100, 20+100*j, 80, 80)
                    if self.current_plant(i,j) is not None:
                        screen.blit(self.current_plant(i,j), (plant_rect.x, plant_rect.y))
